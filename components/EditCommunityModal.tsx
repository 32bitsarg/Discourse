...
interface EditCommunityModalProps {
  ...
  community: {
    ...
    name_changed_at?: string | Date | null
  }
  onSave: () => void
}
...
export default function EditCommunityModal({ isOpen, onClose, community, onSave }: EditCommunityModalProps) {
    ...
    if (community) {
      ...
      if (community.name_changed_at) {
        ...
        if (daysSinceChange < 7) {
          ...
          setNameChangeMessage(`Puedes cambiar el nombre nuevamente en ${daysRemaining} día${daysRemaining > 1 ? 's' : ''}`)
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (name.trim() !== community.name && !canChangeName) {
      alert(nameChangeMessage)
      return
    }

    setIsSaving(true)
    try {
      const res = await fetch(`/api/subforums/${community.id}`, {
        method: 'PUT',
      ...
      alert(error instanceof Error ? error.message : 'Error al actualizar la comunidad')
    } finally {
      setIsSaving(false)
    }
  }
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              <motion.div
                className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[calc(90vh-6rem)] flex flex-col"
                initial={{ scale: 0.9, y: 20 }}
                animate={{ scale: 1, y: 0 }}
                exit={{ scale: 0.9, y: 20 }}
              >
                <div className="flex items-center justify-between p-6 border-b flex-shrink-0">
                  <h2 className="text-2xl font-bold text-gray-900">Editar comunidad</h2>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      ...
                      {!canChangeName && (
                        <span className="ml-2 text-xs text-orange-600 font-normal">
                          (Puedes cambiarlo una vez cada 7 días)
                        </span>
                      )}
                    </label>
                    ...
                    {nameChangeMessage && (
                      <p className="mt-1 text-xs text-orange-600">{nameChangeMessage}</p>
                    )}
                  </div>
